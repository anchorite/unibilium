...
impl<'a> ExtString<'a> {
    ...
    pub fn name(&self) -> &str {
        // Returns static string if called with value between 0 and count
        let name = unsafe { unibilium_sys::unibi_get_ext_str_name(self.term.term, self.index) };
        if name.is_null() {
            panic!(
    ...
    pub fn value(&self) -> Option<&str> {
        let value = unsafe { unibilium_sys::unibi_get_ext_str(self.term.term, self.index) };
        if value.is_null() {
            return None;
        }
        let value = unsafe { CStr::from_ptr(value) };
